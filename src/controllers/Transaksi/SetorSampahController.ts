...
class SetorSampahController extends Routers {
  ...
  async setorSampahNasabah(req: Request, res: Response) {
    try {
      const { kode_sampah, kode_barang, berat ,catatan,kode_nasabah, kode_penimbang, kode_admin } = req.body;
      ...
      if(!kodeNasabah || !kodePenimbang || !kodeBarang || !kodeSampah || !kodeAdmins) return error({ message: "Masukan input yang bena N" }, req.originalUrl, 402, res);
      
      const kodeSetor: string = randomKodeNumber("KS-", kodeNasabah["rw"] , kodeNasabah["rt"]);
      ...
      const bers = await SetorSampah.sum('berat', {where:{kode_nasabah:kodeNasabah!["kode_nasabah"]}})
      const berss = await SetorSampah.sum('total', {where:{kode_nasabah:kodeNasabah!["kode_nasabah"]}})

      console.log(bers);

      const beratsampahnasabah =  await DetailSampahNasabahs.sum("berat_sekarang",{where:{kode_nasabah:kodeNasabah!["kode_nasabah"]}})
      const saldosampahnasabah =  await DetailSampahNasabahs.sum("saldo_sekarang",{where:{kode_nasabah:kodeNasabah!["kode_nasabah"]}})
      ...
      success({rows, updateSampahNasabah , updateSampahBS} , "Succes Setor Sampah!", res);
    } catch (err:any) {
      console.log(err);
  ...
  async setorSampahAdmin(req: Request, res: Response) {
    try {
      const { kode_sampah, kode_barang, berat,catatan,kode_bs, kode_super_admin } = req.body;
      ...
      if(!kodeAdminBS) return error({ message: "Masukan input yang bena N" }, req.originalUrl, 402, res);
      ...
      const kodeSusutSampah: string = randomKodeNumber("KSSBS-", kodeAdminBS["rw"] , kodeAdminBS["rt"]);
      ...
      const beratss =  await SusutSampahAdmins.sum("berat" , {where:{kode_admin_bs:kodeAdminBS!["kode_admin"]}})
      const cekHarga =  await SusutSampahAdmins.sum("harga" , {where:{kode_admin_bs:kodeAdminBS!["kode_admin"]}})
      const cek = await DetailSampahBs.sum("berat", {where:{kode_admin:kodeAdminBS!["kode_admin"]}})
      ...
      success({rows, "Total Yang di peroleh" : harga , updateSampahBS} , "Succes Setor Sampah!", res);
    } catch (err:any) {
      console.log(err);
      error({ error: err.message }, req.originalUrl, 403, res);
    }
